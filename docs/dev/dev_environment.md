...
- `project_name`: the default docker compose project name (default: `nautobot-plugin-nornir`)
...
#14 naming to docker.io/nautobot-plugin-nornir/nautobot:3.1.0-py3.12 done
```
...
ee90fbfabd77   nautobot-plugin-nornir/nautobot:3.1.0-py3.12  "nautobot-server rqw…"   16 seconds ago   Up 13 seconds                                               nautobot_plugin_nornir_worker_1
b8adb781d013   nautobot-plugin-nornir/nautobot:3.1.0-py3.12  "/docker-entrypoint.…"   20 seconds ago   Up 15 seconds   0.0.0.0:8080->8080/tcp, :::8080->8080/tcp   nautobot_plugin_nornir_nautobot_1
d64ebd60675d   nautobot-plugin-nornir/nautobot:3.1.0-py3.12  "mkdocs serve -v -a …"   25 seconds ago   Up 18 seconds   0.0.0.0:8001->8080/tcp, :::8001->8080/tcp   nautobot_plugin_nornir_docs_1
...
So for example, our app is named `nautobot-plugin-nornir`, the command would most likely be `docker logs nautobot_plugin_nornir_nautobot_1 -f`. You can find the name of all running containers via `docker ps`.
...
Or set the `INVOKE_NAUTOBOT_PLUGIN_NORNIR_PYTHON_VER` variable.
...
In the package source, there is the `nautobot_plugin_nornir/app-config-schema.json` file, conforming to the [JSON Schema](https://json-schema.org/) format. This file is used to validate the configuration of the app in CI pipelines.
