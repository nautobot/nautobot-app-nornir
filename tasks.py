...
def docker_compose(context, command, **kwargs):
    ...
    for compose_file in context.nautobot_plugin_nornir.compose_files:
        compose_file_path = os.path.join(
            context.nautobot_plugin_nornir.compose_dir, compose_file
        )
...
def build(context, force_rm=False, cache=True):
    ...
    print(
        f"Building Nautobot with Python {context.nautobot_plugin_nornir.python_ver}..."
    )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    dockerfile_path = os.path.join(
        context.nautobot_plugin_nornir.compose_dir, "Dockerfile"
    )
    base_image = (
