...
NAUTOBOT_VER = os.getenv("NAUTOBOT_VER", "v1.0.0a1")
...
NAME = os.getenv("IMAGE_NAME", "nautobot-plugin-nornir")
PWD = os.getcwd()

COMPOSE_FILE = "development/docker-compose.yml"
COMPOSE_OVERRIDE = "docker-compose.override.yml"
BUILD_NAME = "nautobot_plugin_nornir"
...
def destroy(context, nautobot_ver=NAUTOBOT_VER, python_ver=PYTHON_VER):
    ...
    context.run(
        f"docker volume rm -f {BUILD_NAME}_pgdata_nautobot_plugin_nornir",
        env=DEFAULT_ENV,
    )
...
def makemigrations(context, name="", nautobot_ver=NAUTOBOT_VER, python_ver=PYTHON_VER):
    ...
    if name:
        context.run(
            f"{COMPOSE_COMMAND} run nautobot nautobot-server makemigrations nautobot_plugin_nornir --name {name}",
            env=DEFAULT_ENV,
        )
    else:
        context.run(
            f"{COMPOSE_COMMAND} run nautobot nautobot-server makemigrations nautobot_plugin_nornir",
            env=DEFAULT_ENV,
        )
...
def unittest(context, nautobot_ver=NAUTOBOT_VER, python_ver=PYTHON_VER):
    ...
    docker = f"{COMPOSE_COMMAND} run nautobot"
    context.run(
        f'{docker} sh -c "nautobot-server test nautobot_plugin_nornir"',
        env=DEFAULT_ENV,
        pty=True,
    )
