...
def docker_compose(context, command, **kwargs):
    ...
    if (
        context.nautobot_plugin_nornir.ephemeral_ports
        and context.nautobot_plugin_nornir.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_plugin_nornir.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(os.path.join(context.nautobot_plugin_nornir.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_plugin_nornir.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_plugin_nornir.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_plugin_nornir.python_ver
    dockerfile_path = os.path.join(context.nautobot_plugin_nornir.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_plugin_nornir.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
@task(
    help={
        "version": "Version of Nautobot Nornir Plugin to generate the release notes for.",
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_plugin_nornir'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_plugin_nornir")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_plugin_nornir.migrations"
    ...
    if migrations_dir.is_dir():
        if run_migrations_check:
            migrations_pylint_command = (
                ...
                " nautobot_plugin_nornir.migrations"
            )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_plugin_nornir/templates/"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Nautobot Nornir Plugin."""
    command = "nautobot-server generate_nautobot_plugin_nornir_test_data"
    if database:
